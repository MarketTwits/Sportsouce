package com.markettwits.core_ui.items.theme


import androidx.compose.ui.graphics.Color

object SportSouceColor {
    val SportSouceBlue = Color(8, 32, 75)
    val SportSouceLighBlue = Color(77, 183, 254)
    val SportSouceLightBlueForDarkTheme = Color(100, 180, 240)
    val SportSouceStartEndedPink = Color(247, 92, 150)
    val SportSouceRegistryOpenGreen = Color(94, 207, 177)
    val SportSouceRegistryCommingSoonYellow = Color(248, 189, 56)
    val SportSouceLightRed = Color(239, 84, 80)
    val SportSouceDarkRed = Color(255, 140, 140)
    val VeryLighBlue = Color(244, 250, 254)
    val DirtyWhite = Color(224, 224, 224)
    val SecondaryWhite = Color(247, 247, 247)
    val OnBackgroundLight = Color(61, 60, 60, 255)
    val DarkSecondaryNew = Color(23, 25, 28)
    val DarkSecondaryContainerNew = Color(48, 52, 57)
    val OnPrimaryDark = Color(240, 240, 240)
    val DarkTertiary = Color(235, 235, 235)
    val Gray = Color(100,100,100)
    val LightGray = Color(180, 180, 180)
    val GrayForDarkTheme = Color(160, 160, 160)
    val TelegramIcon = Color(3, 155, 229)
    val VkIcon = Color(0, 120, 255)
    val InstagramIcon = Color(247, 92, 150)
    val WhatsappIcon = Color(19, 153, 10)
}