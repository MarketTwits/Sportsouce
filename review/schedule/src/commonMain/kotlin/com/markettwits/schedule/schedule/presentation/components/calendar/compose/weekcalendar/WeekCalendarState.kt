 ...
 * @param firstVisibleWeekDate the date which will have its week visible
 *     initially.
 * @param firstDayOfWeek the initial value for
 *     [WeekCalendarState.firstDayOfWeek]
 */
@Composable
...
/**
 * A state object that can be hoisted to control and observe calendar
 * properties.
 *
 * This should be created via [rememberWeekCalendarState].
 *
 * @param startDate the desired first date on the calendar. The actual
 *     first date will be the first day in the week to which this date
 *     belongs, depending on the provided [firstDayOfWeek]. Such days will
 *     have their [WeekDayPosition] set to [WeekDayPosition.InDate].
 * @param endDate the desired last date on the calendar. The actual last
 *     date will be the last day in the week to which this date belongs.
 *     Such days will have their [WeekDayPosition] set to
 *     [WeekDayPosition.OutDate].
...
class WeekCalendarState internal constructor(
    ...
    /**
     * The adjusted first date on the calendar to ensure proper alignment of
     * the provided [firstDayOfWeek].
     */
    ...
    /**
     * The desired first date on the calendar. The actual first date will be
     * the first day in the week to which this date belongs, depending on the
     * provided [firstDayOfWeek]. Such days will have their [WeekDayPosition]
     * set to [WeekDayPosition.InDate]
     */
    ...
    /**
     * The desired last date on the calendar. The actual last date will be the
     * last day in the week to which this date belongs. Such days will have
     * their [WeekDayPosition] set to [WeekDayPosition.OutDate]
     */
    ...
    /**
     * The object of [WeekCalendarLayoutInfo] calculated during the last layout
     * pass. For example, you can use it to calculate what items are currently
     * visible.
     *
     * Note that this property is observable and is updated after every
     * scroll or remeasure. If you use it in the composable function it will
     * be recomposed on every change causing potential performance issues
     * including infinity recomposition loop. Therefore, avoid using it in the
     * composition.
     *
     * If you need to use it in the composition then consider wrapping the
     * calculation into a derived state in order to only have recompositions
     * when the derived value changes. See Example5Page in the sample app for
     * usage.
     *
     * If you want to run some side effects like sending an analytics event or
     * updating a state based on this value consider using "snapshotFlow".
     */
    ...
    /**
     * Instantly brings the week containing the given [date] to the top of the
     * viewport.
     *
     ...
     * calendar is being dragged. If you want to know whether the fling (or
    ...
    /**
     * Whether this [ScrollableState] is currently scrolling by gesture, fling
     * or programmatically.
     */
