...
internal object CalendarDefaults {
    /**
     * The default implementation in [rememberSnapFlingBehavior] snaps to the
     * center of the layout but we want to snap to the start. For example, in a
     * vertical calendar, when the layout size is larger than the item size(e.g
     * two or more visible months), we don't want the item's center to be at
     * the center of the layout when it snaps, instead we want the item's top
...
private fun CalendarSnapLayoutInfoProvider(
    ...
    /**
     * In compose 1.3, the default was single page snapping (zero), but this
     * changed in compose 1.4 to decayed page snapping which is not great for
     * calendar usage.
     */
}